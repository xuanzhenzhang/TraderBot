...
const {traderIds} = require("../config/config");
const assert = require("../helpers/assert");

exports.timely_traders_retweet_scheduler = functions.pubsub.schedule('every 5 minutes').onRun((context) => {
    const axios = require('axios');
    const rwClient = require("../clients/twitterApiClient.js");
    const tradersInfoUrl = 'https://us-central1-traderbot-2dbe5.cloudfunctions.net/traders';
    axios.get(tradersInfoUrl)
        .then((tradersInfo) => {
            ...
            tradersInfo.data.forEach((traderInfo)=> {
                ...
                let newTweetsUrl = ''; 
                if (lastTweetId == null) {
                    newTweetsUrl = 'https://api.twitter.com/2/users/' + traderId + '/tweets?max_results=5';
                } else {
                    newTweetsUrl = 'https://api.twitter.com/2/users/' + traderId + '/tweets?since_id=' + lastTweetId;
                }
            ...
            Promise.all(newTweetsInfoPromises)
                .then((results) => {
                    if (results.length != tradersInfo.data.length) {
                        console.log("Length mismatch between traders' info and traders' tweets");
                        return;
                    }
                    for (let i = 0; i < tradersInfo.data.length; i++) {
                        // No new tweets!
                        if (results[i] == null || results[i].meta == null || results[i].meta.result_count == 0) {
                            continue;
                        }
                        ...
                        assert(traderNewTweets != null && traderTweetLatestId != null, 
                        ...
                        traderNewTweets.forEach((traderNewTweet) => {
                            ...
                            */
                            if (traderNewTweetId != lastTweetId && !isReplyTweet) {
                                ...
                                rwClient.post(postUrl,{'tweet_id': traderNewTweetId})
                        ...
                        tradersInfoToUpdate.push({'traderId': traderId, 'lastTweetId': traderTweetLatestId, 'tweetIdList': updatedTweetIdList});
                    }
                    return tradersInfoToUpdate;
                }).then((tradersInfoToUpdate) => {
                    if (tradersInfoToUpdate != null && tradersInfoToUpdate.length == 0) {
                        return;
                    }
                    axios.put(tradersInfoUrl, {'data': tradersInfoToUpdate})
                        .then(() => {})
                        .catch((err) => {return err;});
                }).catch((err) => {
                    return err;
